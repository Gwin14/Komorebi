...
#[cfg(komorebi_videotoolbox)]
extern "C" {
    fn komorebi_encode_hevc_tile(
        pixels: *const u8,
        pixels_len: usize,
        width: u32,
        height: u32,
        pixel_bytes: usize,
        include_parameter_sets: u8,
        output: *mut *mut u8,
        output_len: *mut usize,
    ) -> i32;
    fn komorebi_free_hevc_tile(output: *mut u8, output_len: usize);
}

#[cfg(komorebi_videotoolbox)]
fn encode_videotoolbox_tile(
    pixels: &[u8],
    width: u32,
    height: u32,
    pixel_bytes: usize,
    ...
    let status = unsafe {
        komorebi_encode_hevc_tile(
            pixels.as_ptr(),
            pixels.len(),
            width,
            height,
            pixel_bytes,
            u8::from(include_parameter_sets),
            &mut output,
            &mut output_len,
        )
    };
    if status != 0 || output.is_null() || output_len == 0 {
        return Err(std::io::Error::new(
            ...
            format!("VideoToolbox HEVC encode failed ({status})"),
        ));
    }
    let bytes = unsafe { std::slice::from_raw_parts(output, output_len).to_vec() };
    unsafe { komorebi_free_hevc_tile(output, output_len) };
    Ok(bytes)
}
...
pub fn x265_encode_tiles(
    ...
    #[cfg(komorebi_videotoolbox)]
    {
        return tiles
            ...
            .enumerate()
            .map(|(index, tile)| {
                encode_videotoolbox_tile(tile, width, height, pixel_bytes, index == 0)
            })
            .collect();
    }
    #[cfg(not(komorebi_videotoolbox))]
    {
...
pub fn encode_hevc_tile_gray(pixels: &[u8], width: u32, height: u32) -> std::io::Result<Vec<u8>> {
    ...
    {
        #[cfg(komorebi_videotoolbox)]
        return encode_videotoolbox_tile(pixels, width, height, 1, true);
        #[cfg(not(komorebi_videotoolbox))]
        return encode_raw_tile(pixels, "gray", width, height);
    }
}
...
pub fn encode_hevc_tile_rgb(pixels: &[u8], width: u32, height: u32) -> std::io::Result<Vec<u8>> {
    ...
    {
        #[cfg(komorebi_videotoolbox)]
        return encode_videotoolbox_tile(pixels, width, height, 3, true);
        #[cfg(not(komorebi_videotoolbox))]
        return encode_raw_tile(pixels, "rgb24", width, height);
    }
}
